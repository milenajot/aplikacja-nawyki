...
# --- Połączenie z Bazą Danych (Google Sheets) - NOWA, NIEZAWODNA METODA ---
try:
    scopes = ['https://www.googleapis.com/auth/spreadsheets']
    creds = Credentials.from_service_account_info(st.secrets["gcp_service_account"], scopes=scopes)
    gc = gspread.authorize(creds)
    spreadsheet = gc.open_by_key(st.secrets["connections"]["gsheets"]["spreadsheet"])

    zespol_sheet = spreadsheet.worksheet("Zespol")
    nawyki_sheet = spreadsheet.worksheet("Nawyki")
    wpisy_sheet = spreadsheet.worksheet("Wpisy")
except Exception as e:
    st.error(f"Nie udało się połączyć z Google Sheets. Sprawdź konfigurację. Błąd: {e}")
    st.stop() # Zatrzymujemy aplikację, jeśli nie ma połączenia z bazą danych

# --- Funkcje do Danych (nowa wersja) ---
def load_data():
    try:
        zespol_df = pd.DataFrame(zespol_sheet.get_all_records())
        nawyki_df = pd.DataFrame(nawyki_sheet.get_all_records())
        wpisy_df = pd.DataFrame(wpisy_sheet.get_all_records())

        # Upewniamy się, że puste ramki danych mają poprawne kolumny
        if zespol_df.empty: zespol_df = pd.DataFrame(columns=['ID_Osoby', 'Imie'])
        if nawyki_df.empty: nawyki_df = pd.DataFrame(columns=['ID_Nawyku', 'Opis'])
        if wpisy_df.empty: wpisy_df = pd.DataFrame(columns=['Data', 'ID_Osoby', 'ID_Nawyku', 'Odpowiedz'])

    except gspread.exceptions.WorksheetNotFound:
         st.error("Nie znaleziono odpowiednich arkuszy ('Wpisy', 'Zespol', 'Nawyki') w Twoim pliku Google Sheets.")
         st.stop()
...
def save_dataframe_to_sheet(df, worksheet):
    """Nadpisuje cały arkusz nowymi danymi z DataFrame."""
    worksheet.clear()
...
if strona == "Nasze Postępy":
    ...
    if st.session_state.wpisy_df.empty:
        ...
        dane = pd.merge(st.session_state.wpisy_df, st.session_state.zespol_df, on='ID_Osoby', how='left')
        dane = pd.merge(dane, st.session_state.nawyki_df, on='ID_Nawyku', how='left')
        dane['Data'] = pd.to_datetime(dane['Data'])

        with st.sidebar:
            st.header("Filtry")
            wszystkie_osoby = ['Wszyscy'] + sorted(dane['Imie'].dropna().unique().tolist())
        ...
        # ... (reszta kodu bez zmian, dla zwięzłości pomijam) ...
            ...
            if submitted:
                ...
                for idx, nawyk in st.session_state.nawyki_df.iterrows():
                    ...
                    nowy_wpis = {
                        ...
                        'ID_Osoby': int(id_osoby), 
                        'ID_Nawyku': int(nawyk['ID_Nawyku']), 
                        'Odpowiedz': odpowiedz
                    }
                    nowe_wpisy.append(nowy_wpis)

                nowe_wpisy_df = pd.DataFrame(nowe_wpisy)
                append_dataframe_to_sheet(nowe_wpisy_df, wpisy_sheet)
        ...
        with st.form("dodaj_osobe_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Dodaj osobę") and nowe_imie:
                if nowe_imie not in st.session_state.zespol_df['Imie'].tolist():
                    ...
                    save_dataframe_to_sheet(updated_df, zespol_sheet)
    ...
    with st.expander("Rozwiń, aby zarządzać nawykami"):
        with st.form("dodaj_nawyk_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Dodaj nawyk") and nowy_opis:
                ...
                save_dataframe_to_sheet(updated_df, nawyki_sheet)
            ...
            for index, row in wpisy_pogrupowane.iterrows():
                ...
                if c2.button("Usuń cały wpis", key=f"del_submission_{id_osoby}_{data_wpisu}"):
                    wpisy_po_usunieciu = st.session_state.wpisy_df[
                        ~((st.session_state.wpisy_df['ID_Osoby'].astype(str) == str(id_osoby)) & 
                          (st.session_state.wpisy_df['Data'].astype(str) == data_wpisu))
                    ]
                    save_dataframe_to_sheet(wpisy_po_usunieciu, wpisy_sheet)
